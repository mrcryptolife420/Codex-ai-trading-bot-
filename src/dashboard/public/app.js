  ...
  elements.decisionsList.innerHTML = filtered
    ...
    .map((decision) => {
      ...
      const compactView = `
        ...
        ${decision.paperGuardrailRelief?.length ? `<div class="note-line"><span class="kicker">Paper leniency</span><div class="tag-list">${renderTagList((decision.paperGuardrailRelief || []).slice(0, 3).map(normalizeReasonLabel), "Geen versoepeling")}</div></div>` : ""}
            ...
            return `
              <details class="blocked-card fold-card blocked compact-fold"${detailAttrs(`blocked:${item.symbol || "setup"}:${index}`, false)}>
                ...
                <div class="fold-body compact-body">
                  ...
                  ${item.paperGuardrailRelief?.length ? `<div class="note-line"><span class="kicker">Paper leniency</span><div class="tag-list">${renderTagList((item.paperGuardrailRelief || []).slice(0, REPLAY_RENDER_LIMIT).map(normalizeReasonLabel), "Geen versoepeling")}</div></div>` : ""}
